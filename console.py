...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, args):
        ...
        for keyName, keyValue in kwargs.items():
            setattr(new_instance, keyName, keyValue)
        new_instance.save()
    ...
    def do_all(self, args):
        """Print all instances of a class or all objects."""
        objects = storage.all()
        if args:
            ...
            if cls_name not in HBNBCommand.classes:
                print("** class doesn't exist **")
                return
            objects = {k: v for k, v in objects.items() if k.startswith(cls_name + ".")}


        output = [f"[{v.__class__.__name__}] ({v.id}) {v.to_dict()}" for v in objects.values()]
        for item in output:
            print(f"[{item}]")
